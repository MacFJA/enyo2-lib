...
//Note that the theme is generate only one time and it owned by a enyo.Component subclass

enyo.kind({
	//use "notification" name space for avoid conflict
	name: "notification.*",
	...
	newNotification: function(notification, uid) {
		/*
			...
			is send in "onClose" and "onTap" event :
				this.howYouNameHandler({"notification": notification, "uid": uid});
		*/
	}
});